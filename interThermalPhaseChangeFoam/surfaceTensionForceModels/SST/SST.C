...
void Foam::surfaceTensionForceModels::SST::correct()
{
    ...
    autoPtr<Foam::surfaceTensionModel> sTension
    (
         Foam::surfaceTensionModel::New(surfaceTensionForceProperties_, mesh_)
    );

    //surfaceTensionModel sTension(surfaceTensionForceProperties_, mesh_);

    //fcf = -interface_.sigma()*Kf*deltasf;
    fcf = -fvc::interpolate(sTension->sigma())*Kf*deltasf;
